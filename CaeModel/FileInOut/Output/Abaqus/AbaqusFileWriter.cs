...
namespace FileInOut.Output
{
    ...
    public static class AbaqusFileWriter
    {
                                           ...
                                           CalculixKeyword parent, ConvertPyramidsToEnum convertPyramidsTo)
        {
            if (model.Mesh != null)
            {
                ...
                foreach (var entry in model.Mesh.Parts)
                {
                    ...
                    {
                        // Abaqus elements
                        elementKeyword = new AbqElement(typeEntry.Key, part.Name, typeEntry.Value, convertPyramidsTo);
                ...
                foreach (var entry in model.Constraints)
                {
                    if (entry.Value.Active)
                    {
                        ...
                        else if (entry.Value is RigidBody rb)
                        {
                            ...
                            // Abaqus - Rigid body
                            AbqRigidBody abqRigidBody = new AbqRigidBody(rb, referencePointsNodeIds, surfaceNodeSetName);
                            parent.AddKeyword(abqRigidBody);
                        }
                                        ...
                                        Dictionary<string, int[]> referencePointsNodeIds, CalculixKeyword parent)
        {
            ...
            foreach (var step in model.StepCollection.StepsList)
            {
                // Abaqus - reset solver type
                stepClone = step.DeepClone();
                stepClone.SolverType = SolverTypeEnum.Default;
                //
                if (stepClone is InitialStep) continue;
                else if (stepClone is BoundaryDisplacementStep boundaryDisplacementStep) continue;
                //
                title = new CalTitle(stepClone.Name, "");
                parent.AddKeyword(title);
                //
                CalculixKeyword calStep;
                if (stepClone.Active) calStep = new CalStep(stepClone);
                else calStep = new CalDeactivated(stepClone.Name);
                title.AddKeyword(calStep);
                // Step type
                if (stepClone.Active)
                {
                    if (stepClone.GetType() == typeof(StaticStep) || stepClone is SlipWearStep)
                    {
                        StaticStep staticStep = stepClone as StaticStep;
                        CalStaticStep calStaticStep = new CalStaticStep(staticStep);
                        calStep.AddKeyword(calStaticStep);                        
                    }
                    else if (stepClone is FrequencyStep frequencyStep)
                    {
                        CalFrequencyStep calFrequencyStep = new CalFrequencyStep(frequencyStep);
                        calStep.AddKeyword(calFrequencyStep);
                    }
                    else if (stepClone is ComplexFrequencyStep complexFrequencyStep)
                    {
                    ...
                    else if (stepClone is BuckleStep buckleStep)
                    {
                        CalBuckleStep calBuckleStep = new CalBuckleStep(buckleStep);
                        calStep.AddKeyword(calBuckleStep);
                    }
                    else if (stepClone is ModalDynamicsStep modalStep)
                    {
                    ...
                    else if (stepClone is SteadyStateDynamicsStep steadyStep)
                    {
                    ...
                    else if (stepClone.GetType() == typeof(DynamicStep))
                    {
                        DynamicStep dynamicStep = stepClone as DynamicStep;
                    ...
                    else if (stepClone.GetType() == typeof(HeatTransferStep))
                    {
                        HeatTransferStep heatTransferStep = stepClone as HeatTransferStep;
                        CalHeatTransferStep calHeatTransferStep = new CalHeatTransferStep(heatTransferStep);
                        calStep.AddKeyword(calHeatTransferStep);
                    }
                    else if (stepClone.GetType() == typeof(UncoupledTempDispStep))
                    {
                        UncoupledTempDispStep uncoupledTempDispStep = stepClone as UncoupledTempDispStep;
                    ...
                    else if (stepClone.GetType() == typeof(CoupledTempDispStep))
                    {
                        CoupledTempDispStep coupledTempDispStep = stepClone as CoupledTempDispStep;
                    ...
                    foreach (var parameter in stepClone.StepControls.Parameters)
                    {
                    ...
                    // Abaqus - remove frequency keyword
                }
                else calStep.AddKeyword(new CalDeactivated(stepClone.GetType().ToString()));
                // Boundary conditions title
                title = new CalTitle("Boundary conditions", "");
                calStep.AddKeyword(title);
                // Boundary conditions op
                if (stepClone.Active && !(stepClone is ModalDynamicsStep))
                {
                    title.AddKeyword(new CalOpParameter(OpKeywordEnum.Boundary, OpTypeEnum.New));
                }
                // Boundary conditions
                foreach (var bcEntry in stepClone.BoundaryConditions)
                {
                    if (stepClone.Active && bcEntry.Value.Active)
                        AppendBoundaryCondition(model, stepClone, bcEntry.Value, referencePointsNodeIds, title);
                    else title.AddKeyword(new CalDeactivated(bcEntry.Value.Name));
                }
                // Abaqus BCs
                foreach (var keyword in title.Keywords) if (keyword is CalBC cb) cb.OpType = OpTypeEnum.New;
                // Additional boundary conditions
                AppendAdditionalBoundaryConditions(model, stepClone, additionalBoundaryConditions, referencePointsNodeIds, title);
                // Loads title
                ...
                if (stepClone.Active)
                {
                    if (stepClone.IsLoadTypeSupported(typeof(CLoad)))
                    {
                        title.AddKeyword(new CalOpParameter(OpKeywordEnum.Cload, OpTypeEnum.New));
                    }
                    if (stepClone.IsLoadTypeSupported(typeof(DLoad)))
                    {
                        title.AddKeyword(new CalOpParameter(OpKeywordEnum.Dload, OpTypeEnum.New));
                    }
                    if (stepClone.IsLoadTypeSupported(typeof(CFlux)))
                    {
                        title.AddKeyword(new CalOpParameter(OpKeywordEnum.Cflux, OpTypeEnum.New));
                    }
                    if (stepClone.IsLoadTypeSupported(typeof(DFlux)) && loadTypes.Contains(typeof(DFlux)))
                    {
                        title.AddKeyword(new CalOpParameter(OpKeywordEnum.Dflux, OpTypeEnum.New));
                    }
                    if (stepClone.IsLoadTypeSupported(typeof(FilmHeatTransfer)) && loadTypes.Contains(typeof(FilmHeatTransfer)))
                    {
                        title.AddKeyword(new CalOpParameter(OpKeywordEnum.Film, OpTypeEnum.New));
                    }
                    if (stepClone.IsLoadTypeSupported(typeof(RadiationHeatTransfer)) &&
                ...
                // Loads
                foreach (var loadEntry in stepClone.Loads)
                {
                    if (stepClone.Active && loadEntry.Value.Active) AppendLoad(model, stepClone, loadEntry.Value, referencePointsNodeIds,
                                                                          title);
                    else title.AddKeyword(new CalDeactivated(loadEntry.Value.Name));
                }
                // Abaqus loads
                foreach (var keyword in title.Keywords) if (keyword is CalLoad cl) cl.OpType = OpTypeEnum.New;
                // Defined fields title
                title = new CalTitle("Defined fields", "");
                calStep.AddKeyword(title);
                // Defined fields op
                if (stepClone.Active && stepClone.DefinedFields.Count > 0)
                {
                    title.AddKeyword(new CalOpParameter(OpKeywordEnum.Temperature, OpTypeEnum.New));
                }
                // Defined fields
                foreach (var definedFieldEntry in stepClone.DefinedFields)
                {
                    if (stepClone.Active && definedFieldEntry.Value.Active) AppendDefinedField(model, definedFieldEntry.Value, title);
                ...
                calStep.AddKeyword(title);
                //
                foreach (var historyOutputEntry in stepClone.HistoryOutputs)
                {
                    if (stepClone.Active && historyOutputEntry.Value.Active) AppendHistoryOutput(model, historyOutputEntry.Value,
                                                                                            title);
                ...
                calStep.AddKeyword(title);
                //
                foreach (var fieldOutputEntry in stepClone.FieldOutputs)
                {
                    if (stepClone.Active && fieldOutputEntry.Value.Active)
                        AppendFieldOutput(model, stepClone, fieldOutputEntry.Value, title);
                ...
                else title.AddKeyword(new CalDeactivated(stepClone.Name));
            }
        }
                                                               ...
                                                               CalculixKeyword parent)
        {
            ...
            // Abaqus BCs
            foreach (var keyword in tmp.Keywords) if (keyword is CalBC cb) cb.OpType = OpTypeEnum.New;
            //
            StringBuilder sb = new StringBuilder();
            foreach (var keyword in tmp.Keywords)
            {
        ...
        {
            // Abaqus field outputs
            if (fieldOutput is NodalFieldOutput nfo)
            {
                AbqNodeOutput nodeOutput = new AbqNodeOutput(nfo, step.OutputFrequency);
                parent.AddKeyword(nodeOutput);
            }
            else if (fieldOutput is ElementFieldOutput efo)
            {
                AbqElementOutput elOutput = new AbqElementOutput(efo, step.OutputFrequency);
                parent.AddKeyword(elOutput);
            }
            else if (fieldOutput is ContactFieldOutput cfo)
            {
                AbqContactOutput conOutput = new AbqContactOutput(cfo);
                parent.AddKeyword(conOutput);
            }
            else throw new NotImplementedException();
        }
        //
